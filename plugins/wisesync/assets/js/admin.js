document.addEventListener('DOMContentLoaded', function() {
	...
	function setupMobileMenu() {
	  ...
	  document.addEventListener('click', function(e) {
		if (sidebar && sidebar.classList.contains('sync-mobile-open')) {
		  let clickedInside = e.target.closest('#sync-sidebar') || e.target.closest('#sync-mobile-toggle');
		  if (!clickedInside) {
	...
	function setupDismissibleCards() {
	  ...
	  document.querySelectorAll('.sync-card').forEach(function(card) {
		const cardId = card.dataset.id || 'welcome-card';
		if (localStorage.getItem('sync-dismissed-' + cardId) === 'true') {
	...
	function setupToggleSwitches() {
	  ...
	  toggles.forEach(function(toggle) {
		toggle.addEventListener('change', function() {
		  ...
		  if (toggleId === 'sync-analytics-toggle') {
			if (isChecked) {
