...
class CookieConsent {
    ...
    openCustomizeModal() {
        if (this.elements.customizeModal) {
            ...
            if (functionalCheckbox) functionalCheckbox.checked = this.consentStatus.functional;
            if (analyticalCheckbox) analyticalCheckbox.checked = this.consentStatus.analytical;
            if (advertisingCheckbox) advertisingCheckbox.checked = this.consentStatus.advertising;
            if (trackingCheckbox) trackingCheckbox.checked = this.consentStatus.tracking;
...
function hasConsentCookie() {
    ...
    try {
        ...
        const banners = document.querySelectorAll('.wp-block-sync-cookie-banner');
        if (!banners.length) return true; // If no banner is found, assume cookie is valid
